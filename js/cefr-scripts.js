...
const ENTRIES_KEY = "nylc_cefr_entries";
...
function clampBand(value) {
  ...
  return Math.min(35, Math.max(0, Math.round(num)));
}
...
const TARGETS_KEY = "nylc_cefr_targets";
  ...
  <div class="section-row">
    ...
    <div class="slider-wrap">
      <input type="range" class="ielts-slider" id="slider-${key}" data-key="${key}"
             min="0" max="35" step="1" value="${def}" style="--c:${color}"/>
    </div>
  </div>`;
}

function updateSliderFill(el) {
  const pct = (el.value / 35) * 100;
...
function entryCardHTML(e, col, prev) {
  ...
  const bars = SECTIONS.map(s => `
    <div class="entry-bar-item">
      ...
      <div class="entry-bar-track">
        <div class="entry-bar-fill" style="width:${(e[s.key]/35*100).toFixed(0)}%;background:${s.color}"></div>
      </div>
  ...
  if (prev) {
    ...
    deltaHTML = `<div style="font-size:11px;font-family:'DM Mono',monospace;color:${col2};margin-top:2px">${arrow} ${sign}${diff.toFixed(0)} umumiy</div>`;
  }
...
function loadMaqsadlar() {
  ...
  try {
    ...
    SECTIONS.forEach(s => {
      ...
      parsed[s.key] = Number.isFinite(v) ? Math.min(35, Math.max(0, v)) : fallback[s.key];
    });
    return parsed;
  } catch {
    return fallback;
    ...
    <div class="section-row">
      ...
      <div class="slider-wrap">
        <input type="range" class="target-slider" data-key="${s.key}"
               min="0" max="35" step="1" value="${targets[s.key]}" style="--c:${s.color}"/>
      </div>
    </div>
  `).join("");
