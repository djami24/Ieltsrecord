...
const TIPS = {
  ...
  writing: [
    "2-topshiriq: yozishdan oldin 5 daqiqa rejalashga sarflang - tuzilma g'alaba qozonadi.",
    "Turli xil jumlalar tuzilmasidan foydalaning. Imtihonchilar takrorlanishni sezadi.",
    "Kirish qismida savolni qayta ifodalashtirni mashq qiling.",
  ],
  speaking: [
    "2-qism mavzulariga javob berayotganingizda o'zingizni yozib oling - tanqidiy tinglang.",
    "Javoblarni yodlamang - imtihonchilar biladi. Tabiiy bo'ling.",
...
function normalizeEntry(raw) {
  ...
  entry.umumiy = rawOverall === null ? Math.round((sum / 4) * 2) / 2 : rawOverall;
  return entry;
}
  ...
  <div class="entry-card ${isTahrirlashing ? 'is-editing' : ''}" data-id="${e.id}"
    ...
    <div class="entry-top">
      ...
      <div class="entry-actions">
        ...
        <div class="entry-umumiy">${e.umumiy.toFixed(1)}<span>umumiy</span></div>
      </div>
    </div>
...
function renderInsights() {
  ...
  kpiGrid.innerHTML = `
    ...
    <div class="kpi-card"><div class="kpi-label">Tendensiya</div><div class="kpi-value">${deltaSign}${delta.toFixed(1)}</div></div>
  `;
  ...
  cmp.innerHTML = `
    <div class="tip-text">
      So'nggi test <strong>${latest.umumiy.toFixed(1)}</strong>.
      Hozir zaif soha: <strong style="color:${weakest.color}">${weakest.label} (${weakest.score.toFixed(1)})</strong>.
